...
public class NotificationListAdapter extends ArrayAdapter<NotificationDto_Temp> {
    ...
    private ArrayList<NotificationDto_Temp> items;
    Activity activity;

    public NotificationListAdapter(Context context, ArrayList<NotificationDto_Temp> items){
    ...
    public void setData (ArrayList<NotificationDto_Temp> items){
        this.items = items;
    }
    ...
    @Nullable
    @Override
    public NotificationDto_Temp getItem(int position) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null){
            convertView = inflater.inflate(R.layout.item_notification_list, null);
        ...
        final NotificationDto_Temp item = getItem(position);

        viewHolder.contentView.setText(getContent(item));

        if (item.type == 1){
            viewHolder.actionBtn.setVisibility(View.GONE);
        }else if (item.type == 2){
            viewHolder.actionBtn.setVisibility(View.VISIBLE);
            viewHolder.actionBtn.setText(mContext.getResources().getString(R.string.follow));
            viewHolder.actionBtn.setTextColor(mContext.getResources().getColor(R.color.orange));
        }else if (item.type == 3){
            viewHolder.actionBtn.setVisibility(View.VISIBLE);
            viewHolder.actionBtn.setText(mContext.getResources().getString(R.string.challenge));
            viewHolder.actionBtn.setTextColor(mContext.getResources().getColor(R.color.black));
        }
    ...
    public class ViewHolder {
        ...
        @BindView(R.id.action) Button actionBtn;
